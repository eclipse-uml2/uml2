...
public class BehavioralFeatureOperations extends NamespaceOperations {
   ...
   * @generated
   */
  public static boolean validateAbstractNoMethod(BehavioralFeature behavioralFeature, DiagnosticChain diagnostics,
        ...
        diagnostics.add(new BasicDiagnostic(Diagnostic.ERROR, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString("_UI_GenericInvariant_diagnostic", //$NON-NLS-1$
                new Object[] { "validateAbstractNoMethod", //$NON-NLS-1$
                    org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(behavioralFeature, context) }), new Object[] { behavioralFeature }));
      }
      return false;
    }
    return true;
    ...
    if (n instanceof BehavioralFeature) {
      ...
      for (String nameOfMemberBF : ns.getNamesOfMember(behavioralFeature)) {
        ...
        if (namesOfMemberN.contains(nameOfMemberBF)) {
          Iterator<Parameter> bfParameters = behavioralFeature.getOwnedParameters().iterator();
          Iterator<Parameter> nParameters = ((BehavioralFeature) n).getOwnedParameters().iterator();

          while (bfParameters.hasNext() && nParameters.hasNext()) {

            if (!ParameterOperations.matches(bfParameters.next(), nParameters.next())) {
