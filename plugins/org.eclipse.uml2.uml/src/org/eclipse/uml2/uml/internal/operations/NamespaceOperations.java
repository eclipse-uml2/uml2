...
public class NamespaceOperations extends NamedElementOperations {
  ...
  public static boolean validateMembersDistinguishable(Namespace namespace, DiagnosticChain diagnostics,
    ...
    if (diagnostics != null) {
      ...
      for (String name : nameClashes) {
        ...
        for (NamedElement member : elements) {
          ...
          for (NamedElement otherMember : elements) {
            ...
            if (member != otherMember && !member.isDistinguishableFrom(otherMember, namespace)) {
              ...
              diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
                  ...
                  UMLPlugin.INSTANCE.getString("_UI_Namespace_MemberDistinguishable_diagnostic", //$NON-NLS-1$
      ...
      if (!result) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            UMLValidator.NAMESPACE__MEMBERS_DISTINGUISHABLE,
            UMLPlugin.INSTANCE.getString("_UI_Namespace_MembersDistinguishable_diagnostic", //$NON-NLS-1$
        ...
        diagnostics.add(new BasicDiagnostic(Diagnostic.ERROR, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString("_UI_GenericInvariant_diagnostic", //$NON-NLS-1$
                new Object[] { "validateCannotImportSelf", //$NON-NLS-1$
                    org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(namespace, context) }), new Object[] { namespace }));
      }
      return false;
    }
    return true;
  ...
  public static EList<PackageableElement> getImportedMembers(Namespace namespace) {
    ...
    return new UnionEObjectEList<PackageableElement>((InternalEObject) namespace,
        UMLPackage.Literals.NAMESPACE__IMPORTED_MEMBER, importedMembers.size(), importedMembers.toArray());
  }

  protected static EList<String> getNamesOfMember(Namespace namespace, NamedElement element,
      EList<Namespace> namespaces, EList<String> namesOfMember) {
      ...
      if (namespace.getOwnedMembers().contains(element)) {
        ...
        if (elementImportNames.isEmpty()) {
          ...
          for (PackageImport packageImport : namespace.getPackageImports()) {
            ...
            if (importedPackage != null && importedPackage.visibleMembers().contains(element)) {

              getNamesOfMember(importedPackage, element, namespaces, namesOfMember);
            }
          }
        } else {
  ...
  public static EList<PackageableElement> importMembers(Namespace namespace, EList<PackageableElement> imps) {
    ...
    for (NamedElement ownedMember : ownedMembers) {
      ...
      for (String name : namespace.getNamesOfMember(ownedMember)) {
        ...
        if (members == null) {
          memberNames.put(name, members = new UniqueEList.FastCompare<NamedElement>());
        }
        ...
        if (members.size() > 1) {
          ...
          for (NamedElement member : members) {
            ...
            if (member != imp) {
              ...
              if (memberEClass.isSuperTypeOf(impEClass) || impEClass.isSuperTypeOf(memberEClass)) {
