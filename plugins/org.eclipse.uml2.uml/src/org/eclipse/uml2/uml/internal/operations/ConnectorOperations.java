...
public class ConnectorOperations extends RedefinableElementOperations {
  ...
  public static boolean validateTypes(Connector connector, DiagnosticChain diagnostics, Map<Object, Object> context) {
    ...
    if (connectorType != null) {
      ...
      Iterator<Property> memberEnds = connectorType.getMemberEnds().iterator();
      ...
      while (ends.hasNext() && memberEnds.hasNext()) {
        ...
        if (type == null ? memberEndType != null : !type.conformsTo(memberEndType)) {
    ...
    if (!result && diagnostics != null) {
      diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
          UMLValidator.CONNECTOR__TYPES, UMLPlugin.INSTANCE.getString("_UI_Connector_Types_diagnostic", //$NON-NLS-1$
  ...
  public static ConnectorKind getKind(Connector connector) {
    ...
    for (ConnectorEnd end : connector.getEnds()) {
      ...
      if ((role instanceof Port) && end.getPartWithPort() == null && !((Port) role).isBehavior()) {
      ...
      for (ConnectorEnd e : connector.getEnds()) {
        ...
        if (role != null) {
          ...
          if (!allRoles.contains(role)) {
            ...
            if (!(role instanceof Port) || !(allRoles.contains(e.getPartWithPort()))) {

              if (diagnostics != null) {
                diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
                    UMLValidator.CONNECTOR__ROLES, UMLPlugin.INSTANCE.getString("_UI_Connector_Roles_diagnostic", //$NON-NLS-1$
                        getMessageSubstitutions(context, connector)),
                    new Object[] { connector }));
              }
