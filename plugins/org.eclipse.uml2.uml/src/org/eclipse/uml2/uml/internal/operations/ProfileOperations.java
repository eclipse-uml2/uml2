 ...
 *   Kenn Hussey - 323181, 354452, 535301
...
public class ProfileOperations extends PackageOperations {
   ...
   * @generated NOT
   */
  public static boolean validateMetaclassReferenceNotSpecialized(Profile profile, DiagnosticChain diagnostics,
      Map<Object, Object> context) {
    boolean result = true;

    for (org.eclipse.uml2.uml.Class referencedMetaclass : profile.getReferencedMetaclasses()) {

      for (TreeIterator<EObject> allContents = getAllContents(profile, true, false); allContents.hasNext();) {

        EObject eObject = allContents.next();

        if (eObject instanceof org.eclipse.uml2.uml.Package
            && containsSpecializations((org.eclipse.uml2.uml.Package) eObject, referencedMetaclass)) {
    ...
    if (!result && diagnostics != null) {
      diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
          ...
          UMLPlugin.INSTANCE.getString("_UI_Profile_MetaclassReferenceNotSpecialized_diagnostic", //$NON-NLS-1$
   ...
   * @generated NOT
   */
  public static boolean validateReferencesSameMetamodel(Profile profile, DiagnosticChain diagnostics,
      Map<Object, Object> context) {
    EList<Model> metamodels = new UniqueEList.FastCompare<Model>(profile.getReferencedMetamodels());
    ...
    if (metamodels.size() != 1) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            UMLPlugin.INSTANCE.getString("_UI_Profile_ReferencesSameMetamodel_diagnostic", //$NON-NLS-1$
                getMessageSubstitutions(context, profile)),
            new Object[] { profile }));
      }
  ...
  public static EPackage define(Profile profile) {
    ...
    options.put(UML2EcoreConverter.OPTION__ECORE_TAGGED_VALUES, OPTION__PROCESS);
    options.put(UML2EcoreConverter.OPTION__DUPLICATE_FEATURES, OPTION__PROCESS);
    options.put(UML2EcoreConverter.OPTION__DUPLICATE_FEATURE_INHERITANCE, OPTION__PROCESS);
    options.put(UML2EcoreConverter.OPTION__INVARIANT_CONSTRAINTS, OPTION__IGNORE);
    options.put(UML2EcoreConverter.OPTION__OPERATION_BODIES, OPTION__IGNORE);
    options.put(UML2EcoreConverter.OPTION__PROPERTY_DEFAULT_EXPRESSIONS, OPTION__IGNORE);
   ...
   * @generated NOT
   */
  public static EPackage define(Profile profile, Map<String, String> options, DiagnosticChain diagnostics,
      Map<Object, Object> context) {
    Collection<EPackage> ePackages = convertToEcore(profile, options, diagnostics, context);
    EPackage definition = ePackages.size() == 1 ? ePackages.iterator().next() : null;

    if (definition != null) {
      getEAnnotation(profile, UML2_UML_PACKAGE_2_0_NS_URI, true).getContents().add(0, definition);
    }
  ...
  public static EPackage getDefinition(Profile profile) {
    ...
    if (definition == null) {
      EAnnotation eAnnotation = profile.getEAnnotation(UML2_UML_PACKAGE_2_0_NS_URI);
  ...
  protected static ENamedElement getDefinition(Profile profile, NamedElement namedElement, EPackage profileDefinition) {
    ...
    if (namedElement instanceof Classifier) {
      ENamedElement packageDefinition = getDefinition(profile, ((Classifier) namedElement).getPackage(),
          profileDefinition);
      ...
      if (packageDefinition instanceof EPackage) {
        ...
        if (packageDefinition.eContainer() instanceof EAnnotation) {
          ...
          for (EClassifier eClassifier : ((EPackage) packageDefinition).getEClassifiers()) {
            ...
            EAnnotation eAnnotation = eClassifier.getEAnnotation(UML2_UML_PACKAGE_2_0_NS_URI);

            if (eAnnotation != null && eAnnotation.getReferences().contains(namedElement)) {

              return eClassifier;
            }
          }
        } else {
          return ((EPackage) packageDefinition).getEClassifier(getEcoreName(namedElement));
        }
      }

      return null;
    } else if (namedElement instanceof Property) {
      ENamedElement namespaceDefinition = getDefinition(profile, namedElement.getNamespace(), profileDefinition);
      return namespaceDefinition instanceof EClass
          ? ((EClass) namespaceDefinition).getEStructuralFeature(getEcoreName(namedElement))
          : null;
    } else if (namedElement instanceof EnumerationLiteral) {
      ENamedElement namespaceDefinition = getDefinition(profile, namedElement.getNamespace(), profileDefinition);
      return namespaceDefinition instanceof EEnum
          ? ((EEnum) namespaceDefinition).getEEnumLiteral(getEcoreName(namedElement))
          : null;
    } else if (namedElement instanceof Profile) {

      if (namedElement != profile) {
        EAnnotation eAnnotation = profileDefinition.getEAnnotation(UML2_UML_PACKAGE_4_1_NS_URI);
      ...
      ENamedElement packageDefinition = getDefinition(profile,
          ((org.eclipse.uml2.uml.Package) namedElement).getNestingPackage(), profileDefinition);
      ...
      if (packageDefinition instanceof EPackage) {
        ...
        if (profileDefinition.eContainer() instanceof EAnnotation) {
          ...
          for (EPackage eSubpackage : ((EPackage) packageDefinition).getESubpackages()) {
            ...
            EAnnotation eAnnotation = eSubpackage.getEAnnotation(UML2_UML_PACKAGE_4_0_NS_URI);
      ...
      return getDefinition(profile, ((org.eclipse.uml2.uml.Package) namedElement).containingProfile(),
          profileDefinition);
    } else {
      return null;
    }
   ...
   * @generated NOT
   */
  public static EList<org.eclipse.uml2.uml.Class> getReferencedMetaclasses(Profile profile) {
    ...
    for (ElementImport metaclassReference : profile.getMetaclassReferences()) {
      ...
      if (importedElement instanceof org.eclipse.uml2.uml.Class) {
        referencedMetaclasses.add((org.eclipse.uml2.uml.Class) importedElement);
      }
    }
  ...
  public static EList<Model> getReferencedMetamodels(Profile profile) {
    ...
    for (PackageImport metamodelReference : profile.getMetamodelReferences()) {
      ...
      org.eclipse.uml2.uml.Package importedPackage = metamodelReference.getImportedPackage();
  ...
  protected static EList<Extension> getOwnedExtensions(Profile profile, boolean requiredOnly,
      EList<Extension> ownedExtensions) {

    for (Extension extension : EcoreUtil.<Extension>getObjectsByType(profile.getPackagedElements(),
  ...
  public static EList<Extension> getOwnedExtensions(Profile profile, boolean requiredOnly) {
    return ECollections
        .unmodifiableEList(getOwnedExtensions(profile, requiredOnly, new UniqueEList.FastCompare<Extension>()));
  }
