...
public class BehavioredClassifierOperations extends ClassifierOperations {
        ...
        diagnostics.add(new BasicDiagnostic(Diagnostic.ERROR, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString("_UI_GenericInvariant_diagnostic", //$NON-NLS-1$
                new Object[] { "validateClassBehavior", //$NON-NLS-1$
                    org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(behavioredClassifier, context) }), new Object[] { behavioredClassifier }));
      }
      return false;
    }
    return true;
  ...
  public static EList<Interface> getImplementedInterfaces(BehavioredClassifier behavioredClassifier) {
    return getImplementedInterfaces(behavioredClassifier, new UniqueEList.FastCompare<Interface>());
  }

  protected static EList<Interface> getImplementedInterfaces(BehavioredClassifier behavioredClassifier,
  ...
  public static EList<Interface> getAllImplementedInterfaces(BehavioredClassifier behavioredClassifier) {
    return getAllImplementedInterfaces(behavioredClassifier, new UniqueEList.FastCompare<Interface>());
  }

  protected static EList<Interface> getAllImplementedInterfaces(BehavioredClassifier behavioredClassifier,
    ...
    for (Classifier parent : behavioredClassifier.allParents()) {
      ...
      if (parent instanceof BehavioredClassifier) {
        getImplementedInterfaces((BehavioredClassifier) parent, allImplementedInterfaces);
      }
    }

    for (int i = 0, size = allImplementedInterfaces.size(); i < size; i++) {

      for (Classifier parent : allImplementedInterfaces.get(i).allParents()) {
