...
public class PortOperations extends PropertyOperations {
   ...
   * @generated NOT
   */
  public static boolean validatePortAggregation(Port port, DiagnosticChain diagnostics, Map<Object, Object> context) {
    ...
    if (port.getAggregation() != AggregationKind.COMPOSITE_LITERAL) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            UMLValidator.PORT__PORT_AGGREGATION, UMLPlugin.INSTANCE.getString("_UI_Port_PortAggregation_diagnostic", //$NON-NLS-1$
   ...
   * @generated NOT
   */
  public static boolean validateDefaultValue(Port port, DiagnosticChain diagnostics, Map<Object, Object> context) {

    if (port.getType() instanceof Interface && port.getDefaultValue() != null) {

      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            UMLValidator.PORT__DEFAULT_VALUE, UMLPlugin.INSTANCE.getString("_UI_Port_DefaultValue_diagnostic", //$NON-NLS-1$
                getMessageSubstitutions(context, port)),
            new Object[] { port }));
      }
   ...
   * @generated
   */
  public static boolean validateEncapsulatedOwner(Port port, DiagnosticChain diagnostics, Map<Object, Object> context) {
    ...
    if (false) {
      if (diagnostics != null) {
        diagnostics.add(
            new BasicDiagnostic(Diagnostic.ERROR, UMLValidator.DIAGNOSTIC_SOURCE, UMLValidator.PORT__ENCAPSULATED_OWNER,
                org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString("_UI_GenericInvariant_diagnostic", //$NON-NLS-1$
                    new Object[] { "validateEncapsulatedOwner", //$NON-NLS-1$
                        org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(port, context) }), new Object[] { port }));
      }
      return false;
    }
    return true;
  ...
  public static EList<Interface> getProvideds(Port port) {
    EList<Interface> provideds = port.isConjugated() ? basicRequired(port, false) : basicProvided(port, false);

    return new UnionEObjectEList<Interface>((InternalEObject) port, UMLPackage.Literals.PORT__PROVIDED,
  ...
  public static EList<Interface> getRequireds(Port port) {
    EList<Interface> requireds = port.isConjugated() ? basicProvided(port, false) : basicRequired(port, false);

    return new UnionEObjectEList<Interface>((InternalEObject) port, UMLPackage.Literals.PORT__REQUIRED,
  ...
  protected static EList<Interface> basicProvided(Port port, boolean resolve) {
    return basicProvided(port, resolve, new UniqueEList.FastCompare<Interface>());
  }

  protected static EList<Interface> basicProvided(Port port, boolean resolve, EList<Interface> provided) {
    Type type = (Type) port.eGet(UMLPackage.Literals.TYPED_ELEMENT__TYPE, resolve);

    if (type instanceof Interface) {
      provided.add((Interface) type);
    } else if (type instanceof Classifier) {
      Classifier classifier = (Classifier) port.getType(); // must resolve
      ClassifierOperations.allRealizedInterfaces(classifier, false, provided);
    }
  ...
  protected static EList<Interface> basicRequired(Port port, boolean resolve) {
    return basicRequired(port, resolve, new UniqueEList.FastCompare<Interface>());
  }

  protected static EList<Interface> basicRequired(Port port, boolean resolve, EList<Interface> required) {
    Type type = (Type) port.eGet(UMLPackage.Literals.TYPED_ELEMENT__TYPE, resolve);

    if (type instanceof Classifier) {
      Classifier classifier = (Classifier) port.getType(); // must resolve
      ClassifierOperations.allUsedInterfaces(classifier, resolve, required);
    }
