...
public class ElementOperations extends UMLUtil {
    ...
    if (EcoreUtil.isAncestor(element, element.getOwner())) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            UMLValidator.ELEMENT__NOT_OWN_SELF, UMLPlugin.INSTANCE.getString("_UI_Element_NotOwnSelf_diagnostic", //$NON-NLS-1$
  ...
  public static boolean validateHasOwner(Element element, DiagnosticChain diagnostics, Map<Object, Object> context) {
    ...
    if (element.mustBeOwned() && element.getOwner() == null) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            UMLValidator.ELEMENT__HAS_OWNER, UMLPlugin.INSTANCE.getString("_UI_Element_HasOwner_diagnostic", //$NON-NLS-1$
                getMessageSubstitutions(context, element)),
            new Object[] { element }));
      }
    }
      ...
      for (ProfileApplication profileApplication : package_.getAllProfileApplications()) {
        ...
        if (appliedProfile != null) {
          ...
          for (Stereotype stereotype : appliedProfile.allApplicableStereotypes()) {
            ...
            if (appliedDefinition instanceof EClass && !((EClass) appliedDefinition).isAbstract()) {
  ...
  public static Stereotype getAppliedSubstereotype(Element element, Stereotype stereotype, String qualifiedName) {
    ...
    for (Stereotype appliedSubstereotype : element.getAppliedSubstereotypes(stereotype)) {
      ...
      if (safeEquals(appliedSubstereotype.getQualifiedName(), qualifiedName)) {
      ...
      for (int i = 0, length = segments.length; i < length; i++) {
        ...
        if (segment.indexOf('[') == -1) {
          ...
          eStructuralFeature = eClass
              .getEStructuralFeature(getValidJavaIdentifier(segment.substring(0, segment.indexOf('['))));
        ...
        if (eStructuralFeature != null) {
          ...
          if (length > i + 1) {
            ...
            if (eObject != null) {
              ...
              if (eType instanceof EClass) {
                ...
                if (eStructuralFeature.isMany()) {
                  ...
                  List<EObject> list = (List<EObject>) eObject.eGet(eStructuralFeature);
            ...
            if (eStructuralFeature.isMany()) {
              @SuppressWarnings("unchecked")
              List<Object> list = eObject == null ? Collections.emptyList()
                  : (List<Object>) eObject.eGet(eStructuralFeature);

              return index == -1 ? !list.isEmpty() : !safeEquals(eStructuralFeature.getDefaultValue(), list.get(index));
            } else {
              return eObject == null ? false
                  : !safeEquals(eStructuralFeature.getDefaultValue(), eObject.eGet(eStructuralFeature));
            }
          }
        }
      }
    ...
    for (int i = 0, length = segments.length; i < length; i++) {
      ...
      if (i + 1 < length) {
        ...
        if (eStructuralFeature.isMany()) {
          ...
          value = eObject == null ? eStructuralFeature.getDefaultValue() : eObject.eGet(eStructuralFeature);
        }

        if (eType instanceof EEnum && value instanceof EEnumLiteral) {
          EAnnotation eAnnotation = eType.getEAnnotation(UML2_UML_PACKAGE_2_0_NS_URI);
            ...
            if (references.size() > 0) {
              ...
              if (reference instanceof Enumeration) {
                value = ((Enumeration) reference)
                    .getOwnedLiteral(UMLUtil.UML2EcoreConverter.getOriginalName((EEnumLiteral) value));
              }
            }
          }
        }
      ...
      if (i + 1 < length) {
        ...
        if (eStructuralFeature.isMany()) {
          ...
          if (value == null) {
            ...
            eObject.eSet(eStructuralFeature, value = EcoreUtil.create(eClass));
          }
        ...
        if (newValue != null) {
          ...
          if (eType instanceof EClass) {
            ...
            if (newValue instanceof List<?>) {
              @SuppressWarnings("unchecked")
              Iterator<Object> j = ((List<Object>) newValue).iterator();
            ...
            if (newValue instanceof List<?>) {
              @SuppressWarnings("unchecked")
              List<Object> newList = new ArrayList<Object>((List<Object>) newValue);

              if (eDataType instanceof EEnum) {
                EEnum eEnum = (EEnum) eDataType;

                for (ListIterator<Object> li = newList.listIterator(); li.hasNext();) {
      ...
      if (!eReference.isDerived() && element.eIsSet(eReference)) {
        ...
        if (eClass.isSuperTypeOf(eReferenceType)) {
          ...
          if (eReference.isMany()) {
            @SuppressWarnings("unchecked")
            List<Relationship> values = (List<Relationship>) element.eGet(eReference);
  ...
  public static EList<DirectedRelationship> getSourceDirectedRelationships(Element element, EClass eClass) {
    ...
    for (Iterator<Relationship> directedRelationships = getRelationships(element, eClass)
  ...
  public static EList<String> getKeywords(Element element) {
    ...
    if (eAnnotation != null) {
      ...
      if (!details.isEmpty()) {
        ...
        for (Iterator<Map.Entry<String, String>> d = details.iterator(); d.hasNext();) {
  ...
  public static org.eclipse.uml2.uml.Package getNearestPackage(Element element) {
    return (org.eclipse.uml2.uml.Package) (element instanceof org.eclipse.uml2.uml.Package ? element
  ...
  protected static EClass getDefinition(Element element, Stereotype stereotype, boolean required) {
    ...
    if (stereotype == null) {
      ...
      if (profile == null) {
        if (required) {
          throw new IllegalArgumentException(String.format("stereotype \"%s\" is not in a profile", //$NON-NLS-1$
        ...
        if (package_ == null) {
          if (required) {
            throw new IllegalArgumentException(String.format("element \"%s\" is not in a package", //$NON-NLS-1$
          ...
          if (profileApplication == null) {
            if (required) {
              throw new IllegalArgumentException(String.format("profile \"%s\" is not applied", //$NON-NLS-1$
            ...
            if (appliedDefinition == null) {
              if (required) {
                throw new IllegalArgumentException(String.format("stereotype \"%s\" has no Ecore definition", //$NON-NLS-1$
                    stereotype.getQualifiedName()));
              }
            } else if (!(appliedDefinition instanceof EClass)) {
              if (required) {
                throw new IllegalArgumentException(String.format("stereotype \"%s\" definition is not an EClass", //$NON-NLS-1$
                    stereotype.getQualifiedName()));
              }
            } else {
              ...
              if (eClass.isAbstract()) {
                if (required) {
                  throw new IllegalArgumentException(String.format("stereotype \"%s\" Ecore definition is abstract", //$NON-NLS-1$
                      stereotype.getQualifiedName()));
                }
              } else {
                return eClass;
  ...
  public static boolean isStereotypeApplicable(Element element, Stereotype stereotype) {
    return getDefinition(element, stereotype) != null && getExtension(element, stereotype) != null;
  }
  ...
  protected static EList<EObject> applyAllStereotypes(Element element,
      Map<EClassifier, Map<Stereotype, EClass>> definitions, EList<EObject> stereotypeApplications) {
        ...
        if (stereotypeEntry.getKey().isInstance(element)) {
          ...
          for (Map.Entry<Stereotype, EClass> definitionEntry : stereotypeEntry.getValue().entrySet()) {
            ...
            if (!element.isStereotypeApplied(stereotype)) {
              stereotypeApplications.add(applyStereotype(element, definitionEntry.getValue(), stereotype));
            }
          }
        }
      }
  ...
  protected static EList<EObject> applyAllStereotypes(Element element,
      Map<EClassifier, Map<Stereotype, EClass>> definitions, boolean resolve) {
    EList<EObject> stereotypeApplications = new UniqueEList.FastCompare<EObject>();
    ...
    if (!element.eContents().isEmpty()) {
      ...
      for (TreeIterator<EObject> allContents = EcoreUtil.getAllContents(element, resolve); allContents.hasNext();) {

        EObject eObject = allContents.next();

        if (eObject instanceof Element) {
          applyAllStereotypes((Element) eObject, definitions, stereotypeApplications);
        }
      }
    }
      ...
      for (ProfileApplication profileApplication : package_.getAllProfileApplications()) {
        ...
        if (appliedProfile != null) {
          ...
          for (Extension ownedExtension : appliedProfile.getOwnedExtensions(true)) {
            ...
            org.eclipse.uml2.uml.Class metaclass = ownedExtension.getMetaclass();
              ...
              if (eClassifier != null) {
                ...
                if (stereotype != null) {
                  ...
                  if (appliedDefinition instanceof EClass && !((EClass) appliedDefinition).isAbstract()) {
                    ...
                    Map<Stereotype, EClass> stereotypes = definitions.get(eClassifier);

                    if (stereotypes == null) {
                      definitions.put(eClassifier, stereotypes = new HashMap<Stereotype, EClass>());
                    }
  ...
  public static EObject applyStereotype(Element element, Stereotype stereotype) {
    ...
    if (getExtension(element, stereotype) == null) {
      throw new IllegalArgumentException(String.format("stereotype \"%s\" is not applicable to %s", //$NON-NLS-1$
          stereotype.getQualifiedName(), element.eClass().getName()));
    }

    if (element.getStereotypeApplication(stereotype) != null) {
      throw new IllegalArgumentException(String.format("stereotype \"%s\" is already applied", //$NON-NLS-1$
          stereotype.getQualifiedName()));
    }
  ...
  protected static EList<EObject> unapplyAllNonApplicableStereotypes(Element element,
      EList<EObject> nonApplicableStereotypes) {
  ...
  public static EList<EObject> unapplyAllNonApplicableStereotypes(Element element, boolean resolve) {
    ...
    if (!element.eContents().isEmpty()) {
      ...
      for (TreeIterator<EObject> allContents = EcoreUtil.getAllContents(element, resolve); allContents.hasNext();) {
        ...
        if (eObject instanceof Element) {
          unapplyAllNonApplicableStereotypes((Element) eObject, nonApplicableStereotypes);
        }
      }
    }
  ...
  public static EObject unapplyStereotype(Element element, Stereotype stereotype) {
    ...
    if (element.isStereotypeRequired(stereotype)) {
      throw new IllegalArgumentException(String.format("stereotype \"%s\" is a required extension of %s", //$NON-NLS-1$
          stereotype.getQualifiedName(), element.eClass().getName()));
    }
    ...
    if (stereotypeApplication == null) {
      throw new IllegalArgumentException(String.format("stereotype \"%s\" is not applied", //$NON-NLS-1$
          stereotype.getQualifiedName()));
    }
  ...
  protected static void destroyStereotypeApplication(Element element, EObject stereotypeApplication) {
    destroy(stereotypeApplication);

    String storageName = UMLUtil.isStereotypeApplicationStorageEnabled() ? getStereotypeApplicationStorageID(element)
        : null;
    if (storageName != null) {
  ...
  public static Stereotype getApplicableStereotype(Element element, String qualifiedName) {
    ...
    for (Stereotype applicableStereotype : element.getApplicableStereotypes()) {
      ...
      if (safeEquals(applicableStereotype.getQualifiedName(), qualifiedName)) {
  ...
  protected static void destroy(EObject ancestorEObject) {
    ...
    if (ancestorEObject.eContents().isEmpty()) {
      ...
      if (ancestorEObject instanceof Element) {
        UMLUtil.destroyAll(((Element) ancestorEObject).getStereotypeApplications());
      ...
      for (TreeIterator<EObject> allContents = getAllContents(ancestorEObject, true, false); allContents.hasNext();) {
  ...
  public static EList<Element> allOwnedElements(Element element) {
    return ECollections.unmodifiableEList(allOwnedElements(element, new UniqueEList.FastCompare<Element>()));
  }
