...
public class NamedElementOperations extends ElementOperations {
        ...
        diagnostics.add(new BasicDiagnostic(Diagnostic.ERROR, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString("_UI_GenericInvariant_diagnostic", //$NON-NLS-1$
                new Object[] { "validateHasNoQualifiedName", //$NON-NLS-1$
                    org.eclipse.emf.ecore.util.EObjectValidator.getObjectLabel(namedElement, context) }), new Object[] { namedElement }));
      }
      return false;
    }
    return true;
    ...
    if (namedElement.getNamespace() == null && namedElement.eIsSet(UMLPackage.Literals.NAMED_ELEMENT__VISIBILITY)) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            UMLPlugin.INSTANCE.getString("_UI_NamedElement_VisibilityNeedsOwnership_diagnostic", //$NON-NLS-1$
  ...
  protected static Dependency createDependency(NamedElement namedElement, NamedElement supplier, EClass eClass) {
    org.eclipse.uml2.uml.Package package_ = namedElement.getNearestPackage();
  ...
  public static Dependency createDependency(NamedElement namedElement, NamedElement supplier) {
    return createDependency(namedElement, supplier, UMLPackage.Literals.DEPENDENCY);
  }
  ...
  public static String getLabel(NamedElement namedElement, boolean localize) {
    ...
    return getString(namedElement,
        LABEL_KEY_PREFIX
            + getValidJavaIdentifier(isEmpty(qualifiedName) ? EMPTY_STRING : qualifiedName.replace(':', '_')),
  ...
  public static Usage createUsage(NamedElement namedElement, NamedElement supplier) {
    return (Usage) createDependency(namedElement, supplier, UMLPackage.Literals.USAGE);
  }
  ...
  protected static EList<Namespace> allNamespaces(NamedElement namedElement, EList<Namespace> allNamespaces) {
    ...
    if (owner instanceof TemplateParameter) {
      TemplateSignature signature = ((TemplateParameter) owner).getSignature();
  ...
  public static EList<Namespace> allNamespaces(NamedElement namedElement) {
    return ECollections.unmodifiableEList(allNamespaces(namedElement, new UniqueEList.FastCompare<Namespace>()));
  }
  ...
  public static Namespace getNamespace(NamedElement namedElement) {
    ...
    return eInternalContainer instanceof Namespace
        && NAMESPACE__MEMBER__ALL_SUBSETS.contains(namedElement.eContainingFeature()) ? (Namespace) eInternalContainer
            : null;
  }
  ...
  public static EList<Dependency> getClientDependencies(NamedElement namedElement) {
    ...
    for (EStructuralFeature.Setting nonNavigableInverseReference : getNonNavigableInverseReferences(namedElement)) {
      ...
      if (nonNavigableInverseReference.getEStructuralFeature() == UMLPackage.Literals.DEPENDENCY__CLIENT) {
        clientDependency.add((Dependency) nonNavigableInverseReference.getEObject());
      }
    }

    return new UnionEObjectEList<Dependency>((InternalEObject) namedElement,
        UMLPackage.Literals.NAMED_ELEMENT__CLIENT_DEPENDENCY, clientDependency.size(), clientDependency.toArray());
  }
  ...
  public static EList<org.eclipse.uml2.uml.Package> allOwningPackages(NamedElement namedElement) {
    ...
    while ((namedElement = (NamedElement) getOwningElement(namedElement, UMLPackage.Literals.PACKAGE, true)) != null) {
