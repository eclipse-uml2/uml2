...
public class OperationOperations extends BehavioralFeatureOperations {
    ...
    if (count > 1) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            UMLPlugin.INSTANCE.getString("_UI_Operation_AtMostOneReturn_diagnostic", //$NON-NLS-1$
    ...
    if (operation.getBodyCondition() != null && !operation.isQuery()) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            UMLPlugin.INSTANCE.getString("_UI_Operation_OnlyBodyForQuery_diagnostic", //$NON-NLS-1$
                getMessageSubstitutions(context, operation)),
            new Object[] { operation }));
      }
    }
  ...
  public static boolean isConsistentWith(Operation operation, RedefinableElement redefiningElement) {
    ...
    if (redefiningElement instanceof Operation && redefiningElement.isRedefinitionContextValid(operation)) {
      ...
      if (ownedParametersSize == opOwnedParameters.size()) {
        ...
        for (int i = 0; i < ownedParametersSize; i++) {
          ...
          if (redefiningParam.isOrdered() != redefinedParam.isOrdered()) {

            return false;
          }

          ParameterDirectionKind redefiningDirection = redefiningParam.getDirection();
          ...
          if (redefiningDirection == ParameterDirectionKind.INOUT_LITERAL) {
            ...
            if (!redefiningParam.compatibleWith(redefinedParam) || !redefinedParam.compatibleWith(redefiningParam)) {
