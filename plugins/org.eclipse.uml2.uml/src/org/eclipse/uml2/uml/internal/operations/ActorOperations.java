...
public class ActorOperations extends BehavioredClassifierOperations {
   ...
   * @generated NOT
   */
  public static boolean validateAssociations(Actor actor, DiagnosticChain diagnostics, Map<Object, Object> context) {
    boolean result = true;
    ...
    for (Association association : actor.getAssociations()) {
      ...
      if (result) {
        Property actorEnd = association.getMemberEnd(null, actor).getOtherEnd();
        Type actorEndType = actorEnd == null ? null : actorEnd.getType();
      ...
      if (!result) {
        ...
        if (diagnostics != null) {
          diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
              UMLValidator.ACTOR__ASSOCIATIONS, UMLPlugin.INSTANCE.getString("_UI_Actor_Associations_diagnostic", //$NON-NLS-1$
                  getMessageSubstitutions(context, actor, association)),
   ...
   * @generated NOT
   */
  public static boolean validateMustHaveName(Actor actor, DiagnosticChain diagnostics, Map<Object, Object> context) {
    boolean result = !isEmpty(actor.getName());

    if (!result && diagnostics != null) {
      diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
          UMLValidator.ACTOR__MUST_HAVE_NAME, UMLPlugin.INSTANCE.getString("_UI_Actor_MustHaveName_diagnostic", //$NON-NLS-1$
