...
public class RegionOperations extends NamespaceOperations {
  ...
  public static boolean validateInitialVertex(Region region, DiagnosticChain diagnostics, Map<Object, Object> context) {
    ...
    for (Vertex subvertex : region.getSubvertices()) {
      ...
      if (subvertex instanceof Pseudostate && ((Pseudostate) subvertex).getKind() == PseudostateKind.INITIAL_LITERAL) {
    ...
    if (initialVertexCount > 1) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            UMLValidator.REGION__INITIAL_VERTEX, UMLPlugin.INSTANCE.getString("_UI_Region_InitialVertex_diagnostic", //$NON-NLS-1$
   ...
   * @generated NOT
   */
  public static boolean validateDeepHistoryVertex(Region region, DiagnosticChain diagnostics,
    ...
    if (deepHistoryVertexCount > 1) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            UMLPlugin.INSTANCE.getString("_UI_Region_DeepHistoryVertex_diagnostic", //$NON-NLS-1$
                getMessageSubstitutions(context, region)),
            new Object[] { region }));
      }
   ...
   * @generated NOT
   */
  public static boolean validateShallowHistoryVertex(Region region, DiagnosticChain diagnostics,
      Map<Object, Object> context) {
    ...
    if (shallowHistoryVertexCount > 1) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            ...
            UMLPlugin.INSTANCE.getString("_UI_Region_ShallowHistoryVertex_diagnostic", //$NON-NLS-1$
                getMessageSubstitutions(context, region)),
            new Object[] { region }));
      }
  ...
  public static boolean validateOwned(Region region, DiagnosticChain diagnostics, Map<Object, Object> context) {
    ...
    if (region.getStateMachine() != null && region.getState() != null) {
      ...
      if (diagnostics != null) {
        diagnostics.add(new BasicDiagnostic(Diagnostic.WARNING, UMLValidator.DIAGNOSTIC_SOURCE,
            UMLValidator.REGION__OWNED, UMLPlugin.INSTANCE.getString("_UI_Region_Owned_diagnostic", //$NON-NLS-1$
                getMessageSubstitutions(context, region)),
            new Object[] { region }));
      }
    ...
    if (redefiningElement != null && redefiningElement.isRedefinitionContextValid(region)) {
      ...
      for (Vertex redefiningSubvertex : redefiningRegion.getSubvertices()) {
        ...
        if (redefiningSubvertex instanceof State) {
          State redefinedState = ((State) redefiningSubvertex).getRedefinedState();

          if (redefinedState != null && !allSubvertices.contains(redefinedState)) {
      ...
      for (Transition redefiningTransition : redefiningRegion.getTransitions()) {
        ...
        if (redefinedTransition != null && !allTransitions.contains(redefinedTransition)) {
  ...
  public static boolean isRedefinitionContextValid(Region region, RedefinableElement redefinedElement) {
    ...
    if (redefinedElement instanceof Region) {
      ...
      if (stateMachine == null) {
        State state = region.getState();
        State redefinedState = state == null ? null : state.getRedefinedState();
        ...
        for (StateMachine extendedStateMachine : stateMachine.getExtendedStateMachines()) {
          ...
          if (extendedStateMachine.getRegions().contains(redefinedElement)) {
  ...
  protected static EList<Region> getAllExtendedRegions(Region region) {
    return getAllExtendedRegions(region, new UniqueEList.FastCompare<Region>());
  }

  protected static EList<Vertex> getAllSubvertices(Region region) {
    EList<Vertex> allSubvertices = new UniqueEList.FastCompare<Vertex>(region.getSubvertices());
  ...
  protected static EList<Transition> getAllTransitions(Region region) {
    EList<Transition> allTransitions = new UniqueEList.FastCompare<Transition>(region.getTransitions());
